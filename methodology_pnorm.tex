...
As an alternative to this projected distribution model, one method we might consider would be to map
  ...
  If $x_d \to 0$, then ${\bf y}\to\infty$. \bruno{\bf Did you implement this approach? If not, then drop it as well. You need to focus on the approaches that you took. You may add some discussion at the end. Otherwise the narrative becomes confusing.}

\bruno{An example of the approach of using the angular transformation to obtain a distribution on the hypershere is given by the }i\st{On the} $\mathcal{L}_2$ norm\st{, we consider spherical coordinates}.  Let ${\bf x} \in \mathcal{S}_{2}^{d-1}$.
  ...
  $r$ \st{to create an angular distribution constructed on $[0,\pi/2]^{d-1}$}. \st{But we} \bruno{Alternatively, we} can \st{also construct} \bruno{obtain}
