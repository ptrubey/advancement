...
Assuming $\bm{y} \in \mathcal{S}_{p}^{d-1}$, then for finite $p$, $y_d$ can always be represented
  ...
  unit ball for the $p$ norm. That induces a distribution on the unit $\infty$ norm ball. You can use the
  case of the 2-norm as an example. Intuitively, it seems that using a large $p$ would work better.}
...
The models we present here are obtained by assuming a $d$-dimensional Gamma distribution on $\bm{x}$.
  ...
  $r$ can be integrated out using the kernel of a gamma distribution.\makenote{The rest of this paragraph,
  relevant to posterior analysis of the projected gamma distribution, was repeated in that section.
