\subsection{Applications of EVT Analysis}

\subsubsection{Face Probabilities}
...
\subsubsection{Pairwise Extremal Dependence Coefficients}
A summary measure of extremal dependence between dimensions can be offered by the extremal dependence
  ...
  observations.  For a random variable ${\bf W}$, which has been standardized such that the marginal
  distributions are common between dimensions, it is defined as
  \begin{equation}
    \chi_{kl} = \lim\limits_{u\to\infty}\text{P}\left(W_k > u\mid W_l > u\right).
  \end{equation}
  ...
  a ${\bf W}$ descended from a Pareto process, Warner and Sans{\'o}\cite{warner2018} transform $W$
  to the unit square, and describe another representation of this quantity, such that
  \begin{equation}
  ...
  measure is independent of angular measure.  Important to note, however, is that asymptotic
  independence, where $\chi_{kl} = 0$, can not be described under a Pareto process based model.

\subsubsection{Conditional Survival Curves}
Another interesting inference we can make from a fitted model is a conditional survival curve.
  Following basic rules of conditional probability, we can develop survival curves based on
  threshold exceedences. Assume ${\bf Z} = (Z_1,\ldots,Z_d)$ follows a $d$-variate Pareto distribution.
