...
The posterior predictive loss criterion, \emph{PPL} is introduced in \cite{gelfand1998}.  When we
  ...
  \bruno{This development of the PPLC looks pretty ad hoc. Either you develop an actual multivariate PPLC by looking at the original paper, or the discussion of the PPLC in the Gneiting and Raftery paper (chances are it's already been done, check the literature) or you just stick to the univariate version and take and take an average over all dimensions, which is what people do in spatial statistics most of the time.}
...
Let's go back to that first rotation--we held that as a numerically easier analogue to our actual
  ...
  \bruno{This whole paragraph can be made much shorter and crispier. Introduce the problem, then mention the difficulty of defining an actual distance in $S_\infty^{d-1}$ and jump into the negative kernel idea, then write the lemma from the paper by Gneiting and Raftery, define a suitable kernel and write a proposition proving that it is negative definite, then comment on how you calculate it. BTW, I still don't understand the notation ||a,b||.}
