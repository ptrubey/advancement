...
As stated previously, one approach we might try in order to generate a distribution on
  ...
  \bruno{\bf How is the connection between this model and the Generalize Dirichlet obvious? Actually,
                what is the Generalize Dirichlet?}
...
In the estimation of the parameters of this model, we can quickly arrive at a problem, in that
  ...
  \bruno{\bf This section needs to be tied up with the previous section. You start by saying that,
        following the discussion in the previous section ... Then you make sure that your notation
        is consistent. Here we have $\bm{X}$ and $\bm{Y}$ which had not been defined before, and
         $\theta$ has disappeared. Something that is totally obscure at this point is what the
         observations are.}

\subsubsection{ \bruno{Basic} Model}
The \st{vanilla} \bruno{\bf We consider a basic model that consists of ...} model is the most basic model we present.
