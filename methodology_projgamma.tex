...
As stated previously, one approach we might try in order to generate a distribution on
  ...
  the distribution described \st{by} \bruno{in} \cite{nunez2019} maps this distribution
  ...
  \bruno{\bf How is the connection between this model and the Generalize Dirichlet obvious? Actually, what is the Generalize Dirichlet?}
...
In the estimation of the parameters of this model, we can quickly arrive at a problem, in that
  ...
  \bruno{\bf This section needs to be tied up with the previous section. You start by saying that, following the discussion in the previous section ... Then you make sure that your notation is consistent. Here we have $\bm{X}$ and $\bm{Y}$ which had not been defined before, and $\theta$ has disappeared. Something that is totally obscure at this point is what the observations are.}

\subsubsection{ \bruno{Basic} Model}
The \st{vanilla} \bruno{\bf We consider a basic model that consists of ...} model is the most basic model we present.  In this model, we assume all observations
  are descended from a single distribution.  The shape and rate parameters for the latent
  \text{Gamma} random variables, $\alpha_l,\beta_l$, must be greater than 0, so a logical prior to
...
While the generalized Dirichlet is more flexible than a Dirichlet, and \cite{nunez2019}
  ...
  extension to make to the above model is to assume the data, ${\bf y}$, are \st{descending} \bruno{generated} from a finite
...
Additionally, we investigate a finite mixture extension of the \emph{Projected Restricted Gamma} model,
  ...
  \makenote{I need to say this better}.
...
A final extension we can make to this model is to allow for some structure, or dependence, in the prior
  ...
  algorithm \makenote{this will need an explanation, but a paper isn't prepared...} under parallel
  tempering.\makenote{further explanation needed}.
...
We also present a restricted Gamma form of this model, where we have set $\beta_l := 1$ for all
  ...
  \bruno{\bf I think that 3.2.2 -- 3.2.4 are pretty much the same model with some elaborations. You should be able to present the whole story in a much more condensed fashion.}
...
In our motivating example, we defined our problem on $\mathcal{S}_{\infty}^{d-1}$, the positive orthant
  ...
  establishing a density directly on this geometry.  \makenote{will continue this.  describe impetus
  for comparison on $\mathcal{L}_1$, $\mathcal{L}_2$, and $\mathcal{L}_{\infty}$ norms, and the
  realization that even without a proper density on $\mathcal{L}_{\infty}$, we can still conduct
  inference on shape and rate parameters.}
